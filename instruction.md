Dragwyb Form Builder Plugin Development Roadmap
This document outlines the steps and folder structure to develop the Dragwyb Form Builder Plugin. The plugin allows users to create quizzes, polls, surveys, and contact forms using a custom post type (CPT). It also features drag-and-drop functionality for adding fields to the form and provides live preview functionality via an iframe.

Folder Structure
Below is the folder structure for the Dragwyb Form Builder Plugin:
...
Register plugin actions and shortcodes for frontend integration.
2. Custom Post Type (CPT) for Forms (class-dragwyb-form-builder-post-type.php)
...
Allow forms to be saved as posts with editable metadata (e.g., form title, description, and structure).
3. Admin Panel for Form Creation (class-dragwyb-form-builder-admin.php)
...
Update the live preview of the form as the user interacts with the editor.
4. Frontend Display of Forms (class-dragwyb-form-builder-frontend.php)
...
5. AJAX Handling for Form Submissions (class-dragwyb-form-builder-ajax.php)
...
Return a success or failure message after form submission.
6. Live Form Preview in Admin Panel
...
Create an iframe within the admin panel that renders the form with the changes in real-time.
7. Styling and JavaScript
...
Ensure smooth UX/UI for the form builder.
8. Shortcode Support
...
Ensure the shortcode renders the correct form with all the necessary fields and functionality.
...
Format the email with the form’s submitted data (e.g., name, email, message, etc.).
10. Uninstall Script (uninstall.php)
...
Optionally, remove plugin settings or options saved in the WordPress options table.
11. Autoloading PHP Classes (autoload.php)
