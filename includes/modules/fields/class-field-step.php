...
class Field_Step extends Field_Base {
	...
	public function render_child_fields( $row_field ) {
		...
		$type               = $row_field['type'];
		$field_module_cache = $this->frontend_instance->get_module( $type );

		if ( ! $field_module_cache instanceof Field_Base ) {
			return;
		}

		$field_module_cache->set_the_id( sanitize_text_field( $row_field['_id'] ) );
		$field_module_cache->set_frontend_handler( $this->frontend_instance );
		...
		if ( isset( $row_field['attributes'] ) && ! empty( $row_field['attributes'] ) ) {
			...
			$this->attributes_loop( $attributes, $type, $field_data, $field_module_cache );
			$field_module_cache->set_field_settings( $field_data['attributes'] );
		} else {
			$field_module_cache->set_field_settings( $field_data['attributes'] );
		}

		$field_module_cache->render();
	}
		...
		foreach ( $attributes as $attribute => $value ) {
			if ( $field_control = $field_module_cache->get_control( $attribute ) ) {
				if ( isset( $field_control['type'] ) ) {
					...
					$control_obj = $this->frontend_instance->get_control( $control_type );
